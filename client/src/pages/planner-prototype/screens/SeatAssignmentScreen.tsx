...
export default function SeatAssignmentScreen({ table, seats, guests, allSeats, onAssign, onUnassign, onBack }: SeatAssignmentScreenProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "#9CA3AF" }}>
            {tableSeats.filter(s => s.guestId).length}/{table.capacity} {plannerText.common.seated}
          </div>
        </div>
      </div>
      ...
      {activeTab === "overview" ? (
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "28px 16px 12px" }}>
            ...
            <div style={{ fontSize: 13, color: "#6B7280", marginTop: 12, fontWeight: 500 }}>
              {tableSeats.filter(s => s.guestId).length} / {table.capacity} seated
            </div>
          </div>
          {/* assigned guests list */}
          <div style={{ padding: "0 16px 80px" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 12 }}>Assigned Guests</div>
            {tableSeats.filter(s => s.guestId).length === 0 ? (
              <div style={{ fontSize: 13, color: "#9CA3AF", textAlign: "center", padding: "20px 0" }}>No guests assigned yet.</div>
            ) : (
                  ...
                  return (
                    <div key={seat.id} style={{ display: "flex", alignItems: "center", gap: 10, background: "#FFFFFF", border: "1px solid #E5E7EB", borderRadius: 12, padding: "10px 14px" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#9CA3AF" }}>Seat {seat.seatNumber}</div>
                      </div>
                    </div>
                  );
                })}
        ...
        <>
          ...
          <div style={{ padding: "0 16px 4px" }}>
            <div style={{ fontSize: 11, color: "#9CA3AF", textAlign: "center" }}>{plannerText.seats.tapToAssign}</div>
          </div>
          {/* seat list */}
                  ...
                  <>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: RSVP_COLORS[guest.rsvpStatus] ?? "#9CA3AF", marginTop: 1 }}>
                        {plannerText.rsvp[guest.rsvpStatus as keyof typeof plannerText.rsvp] ?? guest.rsvpStatus}
                      </div>
                    </div>
                    <button
                    ...
                    >
                      <Trash2 size={12} /> {plannerText.seats.remove}
                    </button>
                  </>
                ) : (
                  <>
                    <div style={{ flex: 1, fontSize: 13, color: "#9CA3AF" }}>{plannerText.seats.empty}</div>
                    <button
                    ...
                    >
                      <UserPlus size={12} /> {plannerText.seats.assign}
                    </button>
                  </>
                )}
              </div>
      ...
      <BottomSheet open={pickerOpen} onClose={() => { setPickerOpen(false); setTargetSeatId(null); }} title={plannerText.seats.pickGuest} height="tall">
        <GuestPickerSheet
          guests={guests}
          seats={allSeats}
