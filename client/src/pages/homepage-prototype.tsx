...
function PlannerTabletDashboard({ visible }: { visible: boolean }) {
  ...
  // ── Shared sub-sections (reused in both layouts) ──────────────────────────
  ...
  return (
    <div className="overflow-x-hidden bg-[#fff8ef] text-[#18241d]" style={sansStyle}>
      <style>{`
        ...
        @keyframes ppsFadeUp { from { opacity:0; transform:translateY(28px); } to { opacity:1; transform:translateY(0); } }
        ...
        @media (prefers-reduced-motion:reduce) {
          ...
          .pps-dash-float { animation:none !important; }
          .pps-tg-pulse { animation:none !important; }
        }
      `}</style>
        ...
        >
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="lg:grid lg:grid-cols-[minmax(0,300px)_1fr] lg:items-center lg:gap-14 xl:gap-20">
              ...
              {/* ── Right: [chip col] [tablet] [chip col] ── */}
              <div className={`pps-fade ${plannerVisible ? "pps-vis" : ""} mt-12 lg:mt-0`} style={{ transitionDelay: "0.18s" }}>
                ...
                {/* Mobile: compact tablet centered, then feature grid below */}
