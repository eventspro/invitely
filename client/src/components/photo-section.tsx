      ...
      return (
        <div className="relative group overflow-hidden rounded-xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-t transition-opacity duration-300 opacity-0 group-hover:opacity-100" style={{
            background: config.theme?.colors?.accent ? `linear-gradient(to top, ${config.theme?.colors?.accent}20 0%, transparent 100%)` : undefined
          }}></div>
        </div>
      );
    }
    ...
    >
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8" style={{
          borderColor: config.theme?.colors?.accent ? `${config.theme?.colors?.accent}20` : undefined
          ...
          {uploadStatus && (
            <div className="mb-6 p-4 rounded-lg border" style={{
              backgroundColor: config.theme?.colors?.accent ? `${config.theme?.colors?.accent}20` : undefined,
              borderColor: config.theme?.colors?.accent ? `${config.theme?.colors?.accent}30` : undefined
            }}>
