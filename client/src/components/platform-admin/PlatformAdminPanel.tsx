...
type Tab = 'overview' | 'customers' | 'templates' | 'demo-leads';
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden sm:flex gap-1 bg-gray-100 rounded-xl p-1">
            {(['overview', 'customers', 'templates', 'demo-leads'] as Tab[]).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
              ...
              >
                {t === 'overview' ? '📊 Overview' : t === 'customers' ? '👥 Customers' : t === 'templates' ? '🎨 Templates' : '🎯 Demo Leads'}
              </button>
            ))}
          </nav>
        ...
        {tab === 'demo-leads' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Customers who tried the wedding demo editor</p>
            </div>
            <CustomerEditsAdmin />
          </div>
        )}
